...
from app.adapters.driver.api.dependencies import current_user, CurrentUser
...
async def enqueue_video(file: UploadFile = File(...), fps: int = 1, user: CurrentUser = Depends(current_user)):
...
def get_status(job_id: str, user: CurrentUser = Depends(current_user)):
...
def download(job_id: str, user: CurrentUser = Depends(current_user)):
